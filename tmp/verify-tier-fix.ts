...
async function verifyFix() {
    ...
    for (const count of tierCounts) {
        ...
        const config = JSON.parse(board.config as string || '{}');
    ...
    await syncBoardItems(board.id, cubicConfig as any, { forceTiers: true });
    ...
    if (cubicTierItem) {
        ...
        console.log(`Unit Labor: ${unitLabor} (Expected: 6)`); // Default Cubic labor is 6
