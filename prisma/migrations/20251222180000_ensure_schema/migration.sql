...
-- Postgres doesn't support CREATE INDEX IF NOT EXISTS in all versions, but most recent ones do.
-- For safety in older postgres, we can wrap in DO block or just try.
-- Assuming Postgres 9.5+ which supports IF NOT EXISTS.
...
-- Adding other potentially new columns to Quote if missing (based on schema review)
DO $$
BEGIN
